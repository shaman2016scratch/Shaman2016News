...
for(i = 0; i < newsList.length; i++) {
  ii = newsList[i]
  news = `${news}<div class="project"><h3>${ii.name}</h3><p>${ii.description}</p><p>by ${ii.creator}</p><a href="/Shaman2016News/news/${ii.id}">Открыть статью</a><a href="${ii.post}">Открыть пост</a>`
}
innerHTML('news', news)
